...
int read_while_eq(int pin, int v, unsigned timeout) {
	...
	while (1) {
		...
		unsigned time_diff = current_time - start_time;
...
int hc_sr04_get_distance(hc_sr04_t h, unsigned timeout_usec) {
	...
	int read_while_eq_val_off = read_while_eq(h.echo, 0, timeout_usec);
	if (read_while_eq_val_off == 0) {
		return -1;
	}
	unsigned current_time_off = timer_get_usec();
	int read_while_eq_val_on = read_while_eq(h.echo, 1, timeout_usec);
	if (read_while_eq_val_on == 0) {
		return -1;
	}
	unsigned current_time_on = timer_get_usec();
	unsigned time = current_time_on - current_time_off;
	return time / 148;
}
